...
function resolveRepoRootFromFrontendCwd(cwd: string): string {
  // Next dev usually runs with cwd=frontend. Fallback to cwd if already at repo root.
  const basename = path.basename(cwd);
  return basename === "frontend" ? path.resolve(cwd, "..") : cwd;
}

async function resolveSettleScriptPath(repoRoot: string): Promise<string> {
  const candidates = [
    path.join(repoRoot, "scripts", "v3-settle.sh"),
    path.join(process.cwd(), "..", "scripts", "v3-settle.sh"),
  ];
  ...
  throw new Error("No se encontró scripts/v3-settle.sh o no es ejecutable.");
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    const travelerAddress = sanitizeAddress(body.travelerAddress);
    const repoRoot = resolveRepoRootFromFrontendCwd(process.cwd());
    const scriptPath = await resolveSettleScriptPath(repoRoot);

    const args = [marketId];
    if (travelerAddress) {
      args.push("--traveler", travelerAddress);
    }
    ...
    return NextResponse.json(
      {
        success: ok,
        marketId,
        travelerAddress: travelerAddress || null,
    ...
    return NextResponse.json(
      {
        success: false,
        error: error instanceof Error ? error.message : "Error ejecutando settle local",
      },
      { status: 500 }
    );
  }
